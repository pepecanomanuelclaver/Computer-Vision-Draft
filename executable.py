...
def main():
    ...
    # Select the number of frames that you want to plot
    num_frames = 15
    ...
    for n_frame in range(6,num_frames):
        nframe=n_frame
        ...
        while not glfw.window_should_close(window):
            ...
            glClearColor(0.1, 0.2, 0.8, 1) # donde hay 10 habia 0.1
...
def key_callback(window, key, scancode, action, mods):
    ...
    if pressed_yes>2:
        ...
        key = glfw.KEY_G
        action = glfw.PRESS
    if key == glfw.KEY_G and action == glfw.PRESS and nframe<num_frames:
        pressed_yes=pressed_yes+1
        global cube
        nframe = nframe + 1
        print(nframe)
        sleep(10)
        positions = set_voxel_positions(config['world_width'], config['world_height'], config['world_width'], nframe)
